...
```
[1 Byte Tag] [8 Bytes SteamID64 LE] [Message Body (UTF-8)]
```
...
The binary payload is always:

```text
[0]     tag_byte   : uint8   - message type
[1..8]  steamid64  : uint64  - little-endian SteamID64 of the local player
[9..]   message    : bytes   - UTF-8 message body, not null-terminated
```
...
```python
tag = buf[0]
steamid = struct.unpack_from("<Q", buf, 1)[0]
text = buf[9:].decode("utf-8", errors="replace")
```

`steamid == 0` means Steam is unavailable, not logged in, or the current game is running in LAN mode.
...
- The SteamID64 field is always present, even for non-chat packets; those packets typically use `0`.
...
| `cf_allow_ip` | `*` | Allowed source IP for incoming console commands. Use `*` to allow any (default). |
...
- Parses the fixed `tag + steamid64 + text` packet format and prints `0` SteamIDs as `LAN`.
